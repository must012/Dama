...
public class PlayPig {
	...
	public void playpig() {
		Pigpicture pigpic = new Pigpicture();	//돼지 그림을 받아올 객체 생성
		
		Scanner scan = new Scanner(System.in);
		Random ran = new Random();
		
		System.out.println("짜잔! 알에서 돼지가 태어 났습니다!");
		System.out.println(Playdama.name + " 을(를) 잘 키워주세요!");
			
		while(Playdama.die!=1) {
			...
			if(Playdama.growthLvl == 0){					//성장 1단계일 때 출력될 그림
				if(Playdama.emotion >= 120)			//기분이 높으면 해피
					...
					pigpic.level1Angry();				//기분이 낮으면 앵그리
...
//------------------------------ 행동 스위치 시작 -----------------------------------------------------------------------------------
		...
		case 1:		{			// ---------운동
			...
			if(Playdama.dice<80) {		//다이스 수치조정
				...
				Playdama.dice = 0;}					//다이스 값 80 이하
		
			if(Playdama.dice>=80) {
				Array.healthRan += (ran.nextInt(2));				//Array 클래스의 변수를 랜덤을 이용해 값을 지정
			...
			if (Playdama.dice<80) {
				...
				if (Playdama.hungry<=150){
					...
					if (Playdama.health >= 30 && Playdama.emotion >= 30) {
						...
						System.out.println("♪♪♪~ (" + Playdama.name +"이(가) 좋아한다!)");	}
					...
					if (Playdama.health < 30 || Playdama.emotion < 30) {					
						...
						if (Playdama.health < 30) {
							...
							System.out.println(Playdama.name + "이(가) 배고파 해요..");	}		
				...
				Playdama.dice = 0;			//다이스 80 미만
			}
		
				if(Playdama.dice>=80) {
					Array.eatRan += (ran.nextInt(2));				//Array 클래스의 변수를 랜덤을 이용해 값을 지정
	...
	case 3:{											// 놀아주기
			...
			if (Playdama.dice<80) {
				...
				Playdama.dice = 0;	}		//dice 값 80 미만
		
			if(Playdama.dice>=80) {
				System.out.println(Playdama.name + "이(가) 놀다가 그만...");
				Playdama.die = 1;	}
	...
	case 4 : {			// 가위 바위 보
		...
		Playdama.rspEnd= 0;		//다음 가위바위보를 위한 값 초기화
		break;	}	// case 4 끝
	
	case 5 : {						//스탯 확인
		
		System.out.println(Array.pigLvl[Playdama.growthLvl] + "의 단계인 " + Playdama.name);
		...
		if(Playdama.dayCount==5) {		//데이카운트가 5가 될 때마다 하루가 지나며 수치에 영향
			...
			Situationpic.sleep();		//Situationpic 의 먹는 장면 출력
			}

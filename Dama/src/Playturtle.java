...
public class Playturtle {
	...
	public void playtur() {
		Turtlepic turtle = new Turtlepic();	//거북이 그림을 받아올 객체 생성
		
		Scanner scan = new Scanner(System.in);
		Random ran = new Random();
		
		System.out.println("짜잔! 알에서 거북이가 태어 났습니다!");
		...
		while(Playdama.die==0&&Playdama.growthLvl!=3) {				//die수치가 0 이거나 growthLvl(성장레벨)이 3이 아닌동안 반복)
			...
			if(Playdama.growthLvl == 0){					//성장 1단계일 때 출력될 그림
				if(Playdama.emotion >= 120)			//기분이 높으면 해피
					...
					turtle.level1Angry();				//기분이 낮으면 앵그리
...
//------------------------------ 행동 스위치 시작 -----------------------------------------------------------------------------------
		...
		case 1:		{			// ---------운동
			...
			if(Playdama.dice<80) {		//다이스 값에 따른 출력 변화
		...
		case 2: {				//밥주기
			...
			if (Playdama.health < 30 || Playdama.emotion < 30) {					
				...
				if (Playdama.health < 30) {
					...
					System.out.println(Playdama.name + "(이)에게 밥을 더 줘야겟다...");	}	
	...
	case 3:{										// 놀아주기
		...
		if(Playdama.dice>=80) {
			System.out.print(Playdama.name + "이(가) 놀다가");
...
case 5 : {						//스탯 확인
		...
		System.out.println(Array.turLvl[Playdama.growthLvl] + "의 단계인 " + Playdama.name);
